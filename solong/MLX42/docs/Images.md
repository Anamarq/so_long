...
Images are like a canvas for a painting, they are used to display pixel information onto the window and work with something called inst.
inst are copies of the canvas and let you display an image multiple times throughout the window. The idea behind it is that you already 
...
A change in the image's buffer results in a change for all currently displayed inst.
 ...
 * @param inst An instance carries the X, Y, Z location data.
 * @param count The element count of the inst array.
...
If two inst are on the same z layer and are transparent, the transparency breaks and the inst cut off each other.
To prevent this by default any new inst put onto window will be on their own layer.
