...
mlx_instance_t* mlx_grow_inst(mlx_image_t* img, bool* did_realloc)
{
	mlx_image_ctx_t* const ctx = img->context;
	if (img->count >= ctx->inst_capacity)
	{
		if (ctx->inst_capacity == 0)
			ctx->inst_capacity = img->count;
		else
			ctx->inst_capacity *= 2;
		*did_realloc = true;
		return realloc(img->inst, ctx->inst_capacity * sizeof(mlx_instance_t));
	}
	*did_realloc = false;
	return img->inst;
}
...
int32_t mlx_image_to_window(mlx_t* mlx, mlx_image_t* img, int32_t x, int32_t y)
{
	...
	img->count++;
	bool did_realloc;
	mlx_instance_t* inst = mlx_grow_inst(img, &did_realloc);
	...
	img->inst[index].z = ((mlx_ctx_t*)mlx->context)->zdepth++;
	img->inst[index].enabled = true;
	...
	return (mlx_freen(2, inst, queue), mlx_error(MLX_MEMFAIL), -1);
}
...
void mlx_delete_image(mlx_t* mlx, mlx_image_t* image)
{
	...
	// Delete all inst in the render queue
	mlx_list_t* quelst;
	...
	if ((imglst = mlx_lstremove(&mlxctx->images, image, &mlx_equal_image)))
	{
		...
		mlx_freen(5, image->pixels, image->inst, image->context, imglst, image);
	}
}
